...
if (isset($_POST['part'])) {
    ...
    // set org_eid field to 1 if the employee is in eidiki agogi
    $klados_qry = "SELECT klados FROM $av_emp WHERE id = " . intval($emp_id);
    $klados_res = mysqli_query($mysqlconnection, $klados_qry);
    if ($klados_res && mysqli_num_rows($klados_res) > 0) {
        $klados_row = mysqli_fetch_assoc($klados_res);
        $klados = trim($klados_row['klados']);
        if (
            $klados === 'ΠΕ71' || $klados === 'ΠΕ61' || preg_match('/ΕΑΕ$/u', $klados) ||
            $klados === 'PE71' || $klados === 'PE61' || preg_match('/EAE$/u', $klados)
        ) {
            $org_eid = 1;
        }
    }
...
// if page 2
{
    ...
    if (isset($_POST['duallist'])) {
        ...
        $school_codes = array_filter($sch_arr, function ($value) {
