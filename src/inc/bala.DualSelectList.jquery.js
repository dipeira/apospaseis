 ...
 * https://github.com/balachuang/DualSelectList
 */
 ...
 (function($) {
	...
	$.fn.DualSelectList = function(parameter)
	{
		...
		const CONST_RIGHT = 2;
		const CONST_BOTH  = 3;
		const CONST_FILTER_PLACEHOLDER = 'Φίλτρο';
		...
		this.init = function () {
			...
			this.append(
				...
				'	<input class="dsl-filter-input" style="text" value="' + CONST_FILTER_PLACEHOLDER + '" />' +
			...
			$(document).on('keyup', '#' + thisMainID + ' input.dsl-filter-input', function() {
				var fltText = $(this).val().toUpperCase();
				var tarPanel = null;
