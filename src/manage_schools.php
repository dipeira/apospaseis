...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['insert'])) {
        ...
        $ppysde_name = mysqli_real_escape_string($mysqlconnection, $_POST['ppysde_name']);
        $kwdikos = mysqli_real_escape_string($mysqlconnection, $_POST['kwdikos']);
        $dim = (int) $_POST['dim'];
        $omada = (int) $_POST['omada'];
        $inactive = isset($_POST['inactive']) ? 1 : 0;

        $query = "INSERT INTO $av_sch (name, ppysde_name, kwdikos, dim, omada, inactive) 
                  VALUES ('$name', '$ppysde_name', '$kwdikos', $dim, $omada, $inactive)";
        ...
        $omada = (int) $_POST['omada'];
        $inactive = isset($_POST['inactive']) ? 1 : 0;
        $query = "UPDATE $av_sch SET name='$name', ppysde_name='$ppysde_name', kwdikos='$kwdikos', dim=$dim, omada=$omada, inactive=$inactive WHERE id=$id";
        if (mysqli_query($mysqlconnection, $query)) {
    ...
    <div class="container-fluid">
        ...
        <div class="mt-3">
                    ...
                    <tbody>
                        ...
                        while ($row = mysqli_fetch_assoc($result)):
                            ...
                            <tr>
                                ...
                                <td><?php echo $row['ppysde_name']; ?></td>
                                <td><?php echo $row['kwdikos']; ?></td>
                                <td><?php echo $row['dim'] == 2 ? 'Δημοτικό' : ($row['dim'] == 1 ? 'Νηπιαγωγείο' : 'Άλλο'); ?>
                                </td>
                                <td><?php echo $row['omada']; ?></td>
                                <td><?php echo $row['inactive'] ? '<span class="text-danger">Όχι</span>' : '<span class="text-success">Ναι</span>'; ?>
                                </td>
                                <td>
                                    <a href="manage_schools.php?action=edit&id=<?php echo $row['id']; ?>"
                                        class="btn btn-sm btn-info">Επεξεργασία</a>
                                    <a href="manage_schools.php?action=delete&id=<?php echo $row['id']; ?>"
            ...
            <?php elseif ($action == 'add' || $action == 'edit'):
                $row = ['name' => '', 'ppysde_name' => '', 'kwdikos' => '', 'dim' => 2, 'omada' => 0, 'inactive' => 0];
                ...
                <form action="manage_schools.php" method="post" class="mt-4">
                    ...
                    <div class="form-group mb-3 col-md-6">
                        <label>Όνομα Σχολείου (για PPYSDE)</label>
                        <input type="text" name="ppysde_name" class="form-control"
                            value="<?php echo $row['ppysde_name']; ?>">
                    </div>

                    <div class="form-group mb-3 col-md-4">
                        <label>Κωδικός ΥΠΑΙΘ (7ψήφιος)</label>
                        <input type="text" name="kwdikos" class="form-control" value="<?php echo $row['kwdikos']; ?>"
                            required>
                    </div>
                    ...
                    <div class="form-group mb-3 col-md-4">
                        ...
                        <select name="dim" class="form-control">
                            <option value="2" <?php if ($row['dim'] == 2)
                                echo 'selected'; ?>>Δημοτικό</option>
                            <option value="1" <?php if ($row['dim'] == 1)
                                echo 'selected'; ?>>Νηπιαγωγείο</option>
                            <option value="0" <?php if ($row['dim'] == 0)
                                echo 'selected'; ?>>Άλλο</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group mb-3 col-md-4">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="inactive" id="inactive" <?php if ($row['inactive'])
                                echo 'checked'; ?>>
